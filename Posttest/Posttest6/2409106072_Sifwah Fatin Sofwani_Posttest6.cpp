...
Akun user[30] = {
    {{"", "", "", 0}, "admin", "admin", "admin"},
    {{"Fanny", "Jungle", "Mythic", 2000}, "user", "user", "user"},
    {{"Gatotkaca", "Jungle", "Mythic", 4200}, "Dzaki", "user", "user"},
    {{"Harith", "Jungle", "Mythic", 5000}, "Riva", "user", "user"},
    {{"Phoveus", "EXP", "Mythic", 4600}, "Ridwan", "user", "user"},
    {{"Zhuxin", "Mid", "Mythic", 4500}, "Wahyu", "user", "user"},
};
...
void hapusHero();
void lihatAkunUser();
void daftarHeroDescending();
void lihatMMRFavorit();
void daftarNamaUserAscending();
...
void merge(Akun arr[], int l, int m, int r);
void mergeSort(Akun arr[], int l, int r);
...
void adminMenu() {
    while (true) {
        ...
        cout << "6. Daftar Hero Secara Descending" << endl;
        cout << "7. Daftar MMR hero favorit akun user secara Ascending" << endl;
        cout << "8. Daftar Nama User Secara Ascending" << endl;
        cout << "9. Logout" << endl;
        cout << "Masukkan pilihan anda: ";
        getline(cin, choice);
        cls();
        ...
        if (choice == "1") {
            ...
            daftarHeroDescending();
        } else if (choice == "7") {
            lihatMMRFavorit();
        } else if (choice == "8") {
            daftarNamaUserAscending();
        } else if (choice == "9") {
            quit = true;
            break;
        } else {
...
void userMenu() {
    while (true) {
        ...
        cout << "2. Daftar hero secara Descending" << endl;
        cout << "3. Logout" << endl;
        cout << "Masukkan pilihan anda: ";
        getline(cin, choice);
        cls();
...
bool registerUser() {
    ...
    if (roleChoice == "1") {
        user[jumlahAkun] = {{"", "", "", 0}, newUsername, newPassword, "admin"};
        jumlahAkun++;
        cls();
        ...
        cout << "Masukkan MMR hero favorit: ";
        cin >> mmrInput;
        cin.ignore();

        if (mmrInput < 1) {
            cls();
            cout << "MMR tidak valid! MMR harus lebih dari 0." << endl;
            return false;
        }
        
        user[jumlahAkun] = {{heromain, mainrole, rankakun, mmrInput}, newUsername, newPassword, "user"};
        jumlahAkun++;
        cls();
...
void lihatAkunUser() {
    ...
    // Tampilkan data user (kecuali admin)
    for (int i = 0; i < jumlahAkun; i++) {
        if (user[i].role != "admin") {
            cout << "| " << left << setw(18) << user[i].username 
            << " | " << setw(18) << user[i].isiakun.heromain 
            << " | " << setw(18) << user[i].isiakun.mainrole 
            << " | " << setw(18) << user[i].isiakun.rankakun 
            << " | " << setw(18) << user[i].isiakun.mmr << " |" << endl;
        }
    }
...
void daftarHeroDescending() {
    ...
    for (int i = 0; i < 5; ++i) {
        for (int j = 0; j < 10; ++j) {
            if (listhero[i][j][0][0] != '\0') {
                heroes[count].nama = listhero[i][j][0];
                heroes[count].role = listhero[i][j][1];
                heroes[count].mekanik = listhero[i][j][2];
                heroes[count].posisi = listhero[i][j][3];
                count++;
            }
        }
    }
    ...
    for (int i = 0; i < count - 1; ++i) {
        for (int j = 0; j < count - i - 1; ++j) {
            if (heroes[j].nama < heroes[j + 1].nama) {
                HeroData temp = heroes[j];
                heroes[j] = heroes[j + 1];
                heroes[j + 1] = temp;
            }
        }
    ...
    cout << "+-" << string(18, '-') << "-+-" << string(18, '-') << "-+-" << string(18, '-') << "-+-" << string(18, '-') << "-+" << endl;
    cout << "| " << left << setw(18) << "Nama Hero" << " | " << setw(18) << "Role" << " | "  << setw(18) << "Mekanik" << " | " << setw(18) << "Posisi" << " |" << endl;
    cout << "+-" << string(18, '-') << "-+-" << string(18, '-') << "-+-" << string(18, '-') << "-+-" << string(18, '-') << "-+" << endl;

    for (int i = 0; i < count; ++i) {
        cout << "| " << left << setw(18) << heroes[i].nama << " | " << setw(18) << heroes[i].role << " | "<< setw(18) << heroes[i].mekanik << " | "<< setw(18) << heroes[i].posisi << " |" << endl;
    }
...
void lihatMMRFavorit() {
    ...
    cout << "+-" << string(18, '-') << "-+-" << string(18, '-') << "-+-" << string(18, '-') << "-+-" << string(18, '-') << "-+-" << string(10, '-') << "-+" << endl;
    cout << "| " << left << setw(18) << "Username" 
         << " | " << setw(18) << "Hero Main" 
         << " | " << setw(18) << "Main Role" 
         << " | " << setw(18) << "Rank Akun" 
    ...
    for (int i = 0; i < count; i++) {
        cout << "| " << left << setw(18) << temp[i].username 
             << " | " << setw(18) << temp[i].isiakun.heromain 
             << " | " << setw(18) << temp[i].isiakun.mainrole 
             << " | " << setw(18) << temp[i].isiakun.rankakun 
             << " | " << setw(10) << temp[i].isiakun.mmr << " |" << endl;
    }
...
void daftarNamaUserAscending() {
    ...
    for (int i = 0; i < jumlahAkun - 1; i++) {
        ...
        for (int j = i + 1; j < jumlahAkun; j++) {
            if (user[j].username < user[min].username) {
                min = j;
            }
        }
        // Tukar posisi
    ...
    cout << "+-" << string(18, '-') << "-+" << endl;
    cout << "| " << left << setw(18) << "Username" << " |" << endl;
    cout << "+-" << string(18, '-') << "-+" << endl;

    for (int i = 0; i < jumlahAkun; i++) {
        if (user[i].role != "admin") {
            cout << "| " << left << setw(18) << user[i].username << " |" << endl;
        }
    }
...
void merge(Akun arr[], int l, int m, int r) {
    ...
    while (i <= m && j <= r) {
        if (arr[i].isiakun.mmr < arr[j].isiakun.mmr) {
            temp[k] = arr[i];
            i++;
        } else {
            temp[k] = arr[j];
